...
@WebSocketGateway({ namespace: "auctions-socket", cors: true })
  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  afterInit() {
    this.logger.log("Auctions WS Gateway is up and running");
  }
  ...
  public notifyAuctionStatus(
    auctionId: number,
    statuses: { status: string; value: boolean }[]
  ) {
  ...
  public notifyAuctionDepositedNfts(
    auctionId: number,
    info: { tokenId: string; collectionAddress: string }
  ) {
    this.server.emit(`auction_${auctionId}_depositedNfts`, info);
  }

  public notifyAuctionWithdrawnNfts(
    auctionId: number,
    hasWithdrawnAll: boolean,
    tokenId: string
  ) {
    this.server.emit(`auction_${auctionId}_withdrawnNfts`, {
      hasWithdrawnAll,
      tokenId,
    });
  }
  ...
  public notifyAuctionRevenueWithdraw(
    auctionId,
    totalRevenue: string,
    recipient: string
  ) {
    this.server.emit(`auction_${auctionId}_withdrawnRevenue`, {
      totalRevenue,
      recipient,
    });
  }

  public notifyAuctionSlotCaptured(
    auctionId,
    captureInfo: { sender: string; slotIndex: number }
  ) {
  ...
  public notifyERC721Claimed(
    auctionId: number,
    nftInfo: { claimer: string; slotIndex: number }
  ) {
